...
#include<cstdlib>
#include<assert>
struct threadpool* threadpool_init(int thread_num, int queue_max_job_num)
{
    if (thread_num <= 0 || queue_max_job_num <= 0)
    {
...
int threadpool_add_job(struct threadpool* pool, void* (*callback_function)(void *arg), void *arg)
{
    ...
    while ((pool->queue_cur_job_num == pool->queue_max_job_num) && !(pool->queue_close || pool->pool_close))
    {
...
void* threadpool_function(void* arg)
{
    ...
    {
        pthread_mutex_lock(&(pool->mutex));
        while ((pool->queue_cur_job_num == 0) && !pool->pool_close)   //队列为空时，就等待队列非空
        {
